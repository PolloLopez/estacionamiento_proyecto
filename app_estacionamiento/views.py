...
def mp_iniciar_carga(request):
    ...
    try:
        ...
        messages.error(request, "Ingresá un monto válido mayor a 0.")
        return render(request, "usuarios/mp_cargar_saldo.html")
    ...
    preferencia = {
        "items": [
            {
                "title": f"Carga de saldo — {request.user.correo}",
                "quantity": 1,
        ...
        # MP llama a estas URLs según el resultado del pago
        "back_urls": {
        ...
        # El webhook recibe notificaciones de MP (asíncrono)
    ...
    if resultado["status"] not in (200, 201):
        messages.error(request, "No se pudo crear la preferencia de pago. Intentá de nuevo.")
        return render(request, "usuarios/mp_cargar_saldo.html")
...
def mp_exitoso(request):
    ...
    Acreditamos el saldo usando el payment_id que MP envía por GET.
    """
    from decimal import Decimal
...
def mp_fallido(request):
    messages.error(request, "El pago fue rechazado. Podés intentarlo de nuevo.")
    return render(request, "usuarios/mp_resultado.html", {"estado": "fallido"})


@require_login
def mp_pendiente(request):
    messages.warning(request, "El pago está pendiente de acreditación. Te avisaremos cuando se confirme.")
...
def mp_webhook(request):
    ...
    # MP envía tipo "payment" para pagos
